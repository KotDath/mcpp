...
8. Document streaming initialization:
   - Add comment that McpServer or tool handler calls set_streaming(true) before tool execution
   - Streaming mode is typically enabled when client indicates streaming capability in initialization
   - For HTTP transport, streaming is always available via SSE
   - For stdio transport, streaming uses progress notification mechanism
