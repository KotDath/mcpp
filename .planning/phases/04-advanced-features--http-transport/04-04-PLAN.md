...
Note: Transport is non-owning pointer set by McpServer, following same pattern as RequestContext.
  ...
  <verify>
grep -q "subscribe\|unsubscribe\|notify_updated" src/mcpp/server/resource_registry.cpp && grep -q "UriTemplate::expand" src/mcpp/server/resource_registry.cpp && grep -q "subscriptions_" src/mcpp/server/resource_registry.cpp
  </verify>
  <done>
Resources can be subscribed/unsubscribed by client. notify_updated sends notifications/resources/updated to all subscribers. Templates use UriTemplate::expand. Subscription tracking via subscriptions_ map verified.
  </done>
</task>
