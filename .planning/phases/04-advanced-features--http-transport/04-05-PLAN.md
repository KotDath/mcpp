...
Note: Completion struct can be shared between registries. Consider moving to a common header if both use it.
  ...
  <verify>
grep -q "get_completion" src/mcpp/server/prompt_registry.cpp && grep -q "set_completion_handler" src/mcpp/server/resource_registry.h && grep -q "Completion" src/mcpp/server/resource_registry.h
  </verify>
  <done>
PromptRegistry implements completion handler lookup and invocation. ResourceRegistry has completion support with same API. Completion struct verified in resource_registry.h.
  </done>
</task>
