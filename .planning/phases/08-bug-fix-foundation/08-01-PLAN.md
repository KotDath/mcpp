...
Output: JsonRpcRequest::from_json() static method that validates JSON-RPC 2.0 request structure and returns std::optional<JsonRpcRequest>, plus ParseError struct for detailed error codes (useful for logging). ID extraction for error response construction is deferred to Phase 09.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    NOTE: ID extraction for error responses is deferred to Phase 09 when error responses are constructed.
  </action>
  <verify>grep -q "struct ParseError" src/mcpp/core/json_rpc.h && grep -q "enum class Code" src/mcpp/core/json_rpc.h</verify>
  <done>ParseError struct declared with Code enum, message field, and factory methods</done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    NOTE: ParseError factory methods defined in Task 1 are available for logging in Phase 09.
    ID extraction from malformed requests is deferred to Phase 09 when error responses are constructed.
  </action>
...
3. ParseError struct provides detailed error codes for use in Phase 09
